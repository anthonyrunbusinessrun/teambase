    ...
    if (since) {
      rows = await sql`
        SELECT m.id, m.body, COALESCE(m.reactions,'{}') as reactions,
          m.edited, m.created_at as "createdAt", m.user_id as "userId",
          m.deleted_at as "deletedAt", u.full_name as "userName",
          u.avatar_url as "userAvatar", m.user_id as "fromUserId",
