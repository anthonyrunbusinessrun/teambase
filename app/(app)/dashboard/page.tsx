...
async function getDashboardData(userId: string) {
  const [myTaskCount, onlineCount, recentTasks] = await Promise.all([
    db.select({ count: count() }).from(tasks)
      .where(and(eq(tasks.assignedTo, userId), ne(tasks.status, "done"), ne(tasks.status, "cancelled"))),
  ...
  return { myTaskCount: myTaskCount[0]?.count ?? 0, onlineCount: onlineCount[0]?.count ?? 0, recentTasks };
}
...
export default async function DashboardPage() {
  ...
  const nextDeadline = recentTasks.filter(t => t.dueDate && t.dueDate > new Date())
      ...
      <div className="page-content space-y-5">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label:"In Progress",    value:recentTasks.filter(t=>t.status==="in_progress").length, href:"/tasks", color:"hsl(220 57% 40%)" },
        ...
        {nextDeadline?.dueDate && (
          <div className="card-accent p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="font-medium text-foreground truncate mt-0.5" style={{ fontFamily:"'Barlow',sans-serif" }}>{nextDeadline.title}</p>
            </div>
            <div className="text-right flex-shrink-0">
              <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:"1.5rem", lineHeight:1 }}>{timeUntil(nextDeadline.dueDate)}</div>
              <p className="label-caps mt-0.5">{nextDeadline.dueDate.toLocaleDateString("en-US",{month:"short",day:"numeric"})}</p>
            </div>
          </div>
        )}
        ...
        <div className="card-base overflow-hidden">
          ...
          {recentTasks.length === 0 ? (
            ...
            const isUrgent = task.dueDate && !isOverdue && (task.dueDate.getTime()-Date.now()) < 86400000*2;
            return (
              <Link key={task.id} href={`/tasks/${task.id}`}
                className="flex items-center gap-0 border-b border-border last:border-0 hover:bg-muted/20 transition-colors block">
                <span className="w-1 self-stretch flex-shrink-0" style={{ background:PRIORITY_COLOR[task.priority] }}/>
                <div className="flex items-center gap-3 px-4 py-3 flex-1 min-w-0">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground truncate" style={{ fontFamily:"'Barlow',sans-serif" }}>{task.title}</p>
                    <div className="flex items-center gap-3 mt-0.5">
                      <span className="label-caps">{STATUS_LABEL[task.status]}</span>
                      {task.assigneeName && <span className="label-caps text-muted-foreground">→ {task.assigneeName}</span>}
                    </div>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0">
                    {task.dueDate && (
                      <div className="text-right hidden sm:block">
                        <div className="label-caps" style={{ color:isOverdue?"hsl(0 72% 51%)":isUrgent?"hsl(38 90% 50%)":"hsl(var(--muted-foreground))", fontSize:"0.65rem", fontWeight:700 }}>
